...
public class BalanceFetcher : MonoBehaviour
{
    ...
    private void UpdateWalletUI(string balanceStr)
    {
        ...
        {
            walletBalanceText.text = balanceDecimal.ToString("F3", CultureInfo.InvariantCulture);
        }
        else
        {
    ...
    public void SetScoreTo2000()
    {
        PlayerPrefs.SetInt("TotalScore", 2000);
        PlayerPrefs.Save(); // lưu thay đổi xuống bộ nhớ
        Debug.Log("TotalScore has been set to 2000");
        FetchAllBalances();
    }
}
